interface Annotation {
  ...
  day: number;
  time: string;
  // Add other properties of the annotation object here
}
...
function createToolbar() {
  ...
  Fontcolorhighlight.onclick = function () {
    ...
    FontcolorInput.style.setProperty(
      "display",
      FontcolorInput.style.display === "none" ? "inline-block" : "none",
      "important"
    );
  };
  ...
  FontStyle.addEventListener("change", (event) => {
    ...
    if (firstChild?.textContent==="Font Style") {
  ...
  textboxButton.onclick = function () {
    ...
    textInputContainer.style.setProperty(
      "display",
      textInputContainer.style.display === "none" ? "block" : "none",
      "important"
    );
  };
  ...
  searchResultContainer.style.setProperty(
    "border",
    "1px solid #D1D5DB",
    "important"
  ); // Tailwind CSS color border-gray-300
  searchResultContainer.style.setProperty(
    "border-radius",
    "0.25rem",
    "important"
...
const cardStyle = `
  ...
  border-radius: 0.25rem;
`;
function searchAndFilterAnnotations(
  keyword: string | null,
  filter: string | null,
  sort: string | null
) {
  let filteredAnnotations: Annotation[] = allAnnotations;
  ...
  if (keyword) {
    ...
    if (filter === "text") {
      filteredAnnotations = filteredAnnotations.filter((annotation) =>
        regex.test(annotation.text)
      );
    } else if (filter === "note") {
      filteredAnnotations = filteredAnnotations.filter(
        (annotation) =>
          typeof annotation.note_text === "string" &&
    ...
    if (sort === "latest") {
      ...
      filteredAnnotations = sortAnnotationsalphabetically(
        filteredAnnotations,
        "text"
      );
    }
